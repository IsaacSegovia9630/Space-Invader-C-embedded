...
extern const uint8_t LCD_nokia_sprites[][10];
extern const uint8_t numbers[][5];
extern const uint8_t Laser[][2];
...
void LCD_nokia_init(void)
{
	...
	PORT_SetPinMux(PORTC, DATA_OR_CMD_PIN, kPORT_MuxAsGpio); //PTC5
	PORT_SetPinMux(PORTC, RESET_PIN, kPORT_MuxAsGpio);       //PTC7

	GPIO_PinInit(GPIOC, DATA_OR_CMD_PIN, &led_config);
	GPIO_PinInit(GPIOC, RESET_PIN, &led_config);
...
void LCD_nokia_send_sprite(uint8_t sprite)
{
	uint16_t index = 0;
	...
	for (index = 0 ; index < 10 ; index++)
	  LCD_nokia_write_byte(LCD_DATA, LCD_nokia_sprites[sprite][index]);
	//0x20 is the ASCII character for Space (' '). The font table starts with this character

	LCD_nokia_write_byte(LCD_DATA, 0x00); //Blank vertical line padding
}

void LCD_nokia_send_sprite_Laser(uint8_t sprite)
{
	uint16_t index = 0;
	...
	for (index = 0 ; index < 2 ; index++)
	  LCD_nokia_write_byte(LCD_DATA, Laser[sprite][index]);
	//0x20 is the ASCII character for Space (' '). The font table starts with this character

	LCD_nokia_write_byte(LCD_DATA, 0x00); //Blank vertical line padding
}

void LCD_nokia_send_num(uint8_t num)
{
	uint16_t index = 0;
	...
	for (index = 0 ; index < 5 ; index++)
	  LCD_nokia_write_byte(LCD_DATA, numbers[num][index]);
	//0x20 is the ASCII character for Space (' '). The font table starts with this character
